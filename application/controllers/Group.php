...
class Group extends Admin_Controller {
	...
	{ 
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			$update = $this->groups_model->update($data,array("groups.id"=>$group_id)); 
			if ($update)
			{  
        ...
        if(!empty($datas))
        {
            ...
            {   
            	...
            	if($this->data['is_can_edit'] && $data->is_deleted == 0){
            		$edit_url = "<a href='".base_url()."group/edit/".$data->id."' class='btn btn-sm white'><i class='fa fa-pencil'></i> Edit</a>";
            	}  
