...
class produk extends Admin_Controller {
	...
	{ 
		...
		{ 
			...
			{ 
				$this->session->set_flashdata('message', "Success Create produk");
				redirect("produk");
			}
			else
			{
				$this->session->set_flashdata('message_error',"Failed Create produk");
				redirect("produk");
			}
		}else{  
	...
	{ 
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			$update = $this->produk_model->update($data,array("id"=>$this->input->post('id')));
			if ($update)
			{ 
		...
		else
		{
			if(!empty($_POST)){ 
				...
				$data = $this->produk_model->getAllById(array("id"=>$this->data['id'])); 
        ...
        { 
            ...
            {  
            	...
            	if($this->data['is_can_edit'] && $data->is_deleted == 0){
            		$edit_url = "<a href='".base_url()."produk/edit/".$data->id."' class='btn btn-sm white edit'><i class='fa fa-pencil'></i> Edit</a>";
            	}  
