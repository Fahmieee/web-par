...
class Workorder extends Admin_Controller {
	...
	public function index()
	{
		...
		if($this->data['is_can_read']){
			$this->data['content'] = 'admin/workorder/list_v'; 	
		}else{
    ...
    { 
      ...
      if(!empty($data_order)){ 
        ...
        sendNotification(
              ...
              "telah selesai proses ".$service_type, 
              "CLOSED_WO", 
              "FCM_PLUGIN_ACTIVITY",
              $data_order
            ); 
 	...
 	public function dataList()
  {
    $columns = array( 
      ...
      $limit = 0;
      $start = 0;
      $totalData = $this->orders_model->getCountAllWorkorderBy($limit,$start,$search,$order,$dir,$where); 
      ...
      if(!empty($searchColumn[4]['search']['value'])){
        ...
        $isSearchColumn = true;
        $search["DATE_FORMAT(orders.order_date,'%Y-%m-%d')"] = $value;
      } 


      if($isSearchColumn){ 
          $totalFiltered = $this->orders_model->getCountAllWorkorderBy($limit,$start,$search,$order,$dir,$where); 
      }else{
      ...
      $datas = $this->orders_model->getAllWorkorderBy($limit,$start,$search,$order,$dir,$where);
      ...
      { 
        ...
        {  
          ...
          $view_url = "<a href='".base_url()."workorder/view_wo/".$data->id."' class='btn btn-sm white'><i class='fa fa-detail'></i> View WO</a>";
           // if($data->status == 3){
           //    $close_url = "<a href='".base_url()."workorder/close_wo/".$data->id."' class='btn btn-sm white'><i class='fa fa-detail'></i> Close WO</a>";
           // }
                
          $nestedData['id'] = $start+$key+1;
          $nestedData['order_no'] = $data->order_wo_no; 
  ...
  public function report_view_wo()
  { 
    $this->form_validation->set_rules('order_id',"Order Harus Diisi", 'trim|required');   
    ...
    { 
      ...
      if(!empty($_POST)){ 
        ...
        if($orders[0]->service_type == "treatment"){
          $this->data['content'] = 'admin/report/view_wo_v'; 

        }elseif ($orders[0]->service_type == "emergency") {
           
          $this->data['content'] = 'admin/report/view_wo_emergency_v'; 
        }else{
          $this->data['content'] = 'admin/report/view_wo_repair_v'; 
        }
        $this->load->view('admin/layouts/page',$this->data); 
      }
    }
  ...
  { 
    ...
    { 
      ...
      if(!empty($_POST)){ 
        ...
        if($orders[0]->service_type == "treatment"){
          $this->data['content'] = 'admin/workorder/view_wo_v'; 

        }elseif ($orders[0]->service_type == "emergency") {
           
          $this->data['content'] = 'admin/workorder/view_wo_emergency_v'; 
        }else{
          $this->data['content'] = 'admin/workorder/view_wo_repair_v'; 
        }
        $this->load->view('admin/layouts/page',$this->data); 
      }
    }
  }  
  public function reportDataList()
	{
		$columns = array( 
            0 =>'id', 
            1 =>'orders.order_no',
      ...
      { 
        ...
        {  
          ...
          $edit_url = "";
      	  ...
      	  $view_url = "<a href='".base_url()."workorder/report_view_wo/".$data->id."' class='btn btn-sm white'><i class='fa fa-detail'></i> View WO</a>";
           if($data->status == 3){
              $close_url = "<a href='".base_url()."workorder/close_wo/".$data->id."' class='btn btn-sm white'><i class='fa fa-detail'></i> Close WO</a>";
           }
