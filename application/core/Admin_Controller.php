...
class Admin_Controller extends CI_Controller {
 	public function __construct()
	{
		...
		if ($this->data['is_superadmin'])
		{
			...
			$dataPrivilleges = $this->privilleges_model->getAllById(array(
			...
			$this->data['is_can_rescheduled'] = ($this->isInPrivilleges($dataPrivilleges,7));
		}   

 		$this->load->model('notification_model');
		$role_id = $this->data['users_groups']->id;
		$this->data['notifications'] = $this->notification_model->getNotifications(array('to'=>$role_id,'is_read'=>0));
	...
	private function loadMenu(){ 
		...
		if(!empty($this->uri->segment(2))){ 
			if($this->uri->segment(2) !== "create" && $this->uri->segment(2) !== "edit"  && $this->uri->segment(2) !== "detail"){
	 	...
	 	$data_menu = $this->menu_model->getDetailMenuBy(array("url"=>$path_active_name));
	   	$this->data['path_active_name'] = $path_active_name;
