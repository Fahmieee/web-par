...
class Ion_auth
{
	...
	{ 
		if ($this->ion_auth_model->forgotten_password($identity))
		{
			...
			if ($user)
			{
				...
				{
					$link_forgot =  (anchor('auth/reset_password/'. $data['forgotten_password_code'], lang('email_forgot_password_link')));
					$this->sendEmail($user->username, $user->email, $link_forgot);
	...
	/*public function sendEmail($username, $email, $code){
        ...
        curl_setopt($ch, CURLOPT_URL,"http://pcm.shirobyte.com/api/auth/send_email_verification");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS,"username=".$username."&email=".$email."&code=".$code); 
    ...
    {            
        ...
        $CI->email->from('developer@ndt-dev.com', 'Reset Password');
        $CI->email->to($email); 
        ...
        if ($CI->email->send())
        {
            $CI->_response['status'] = TRUE;
            $CI->_response['message'] = "";
            $CI->_response['data'] = "";
        }
        else
        {
             $CI->_response['message'] = $this->email->print_debugger();
        } 
    } 
