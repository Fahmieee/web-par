...
class Ion_auth_model extends CI_Model
{
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			{
				$this->trigger_events(array('post_change_password', 'post_change_password_successful'));
				$this->set_error('post_change_password_successful');
			}
			else
			{
				$this->trigger_events(array('post_change_password', 'post_change_password_unsuccessful'));
	...
	public function errors()
	{
		...
		foreach ($this->errors as $error)
		{
			...
			$_output .= $this->error_start_delimiter . $errorLang . $this->error_end_delimiter." AND ";
		}

		return substr($_output,0, -5) ;
	}
	...
	public function getUsersBy($where){ 
	...
	public function activateUser($code){
		...
		$this->db->update($this->tables['users'], $data, array('code' => $code));
	}

	public function activateForgotenUser($code){
		$data = array(
			"active"=>1
		);
		$this->db->update($this->tables['users'], $data, array('forgotten_password_code' => $code));
	...
	public function change_password_without_old($identity, $new)
	{
		$this->trigger_events('pre_change_password');
